...
describe('NFT', () => {
  ...
  describe('Minting', () => {
    ...
    describe('Failure', async () => {
        ...
        it('does not allow more NFTs to be minted than the max amount', async () => {
            const ALLOW_MINTING_ON = Date.now().toString().slice(0, 10) // Now
            const NFT = await ethers.getContractFactory('NFT')
            nft = await NFT.deploy(NAME, SYMBOL, COST, MAX_SUPPLY, ALLOW_MINTING_ON, BASE_URI)
        ...
        it('does notreturn URIs for invalid tokens', async () => {
            const ALLOW_MINTING_ON = Date.now().toString().slice(0, 10) // Now
            const NFT = await ethers.getContractFactory('NFT')
            nft = await NFT.deploy(NAME, SYMBOL, COST, MAX_SUPPLY, ALLOW_MINTING_ON, BASE_URI)
            nft.connect(minter).mint(100, { value: COST  })
  ...
  describe('Withdraw', () => {
        ...
        describe('Success', async () => {
            ...
            it('emits a withdraw event', async () => {
                expect(transaction).that.emit(nft, 'Withdraw')
        ...
        describe('Failure', async () => {
            ...
            it('prevents non-owner from witdrawing', async () => {
                const ALLOW_MINTING_ON = Date.now().toString().slice(0, 10) // Now
                const NFT = await ethers.getContractFactory('NFT')
                nft = await NFT.deploy(NAME, SYMBOL, COST, MAX_SUPPLY, ALLOW_MINTING_ON, BASE_URI)
